...
1. **Configuration Setup**
   - Configure both `Lambdas.Recipe` and `Lambdas.Authorizer` to load settings from:
     - `appsettings.json`
     - Environment variables (override JSON where applicable).

2. **Lambda Hosting**
   - Configure `Lambdas.Recipe` to run using `AddAWSLambdaHosting(LambdaEventSource.RestApi)`
